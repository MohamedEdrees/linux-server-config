...
##### Link to [Item Catalog](https://github.com/MohamedEdrees/item-catalog-project-FSND.git)

  ```
    User grader
    Pass 11111
    IP Address: 35.159.30.174
    SSH Port: 2200
  ```
...
#### 2. Launch Virtual Machine and SSH into the server.
 ...
 ```
     $ chmod 600 /(address_to_private_key)/Lightsail.rsa
 ```

  * SSH into the instance:

  ```
     $ ssh -i /(address_to_private_key)/Lightsail.rsa root@PUBLIC_IP_ADDRESS
  ```
...
#### 13. Clone the Catalog app from Github
  ...
  * Clone the *SongCatalog* to the catalog directory:
    ...
    ```
     $ git clone https://github.com/MohamedEdrees/item-catalog-project-FSND.git catalog
  ...
  * Make a catalog.wsgi file to serve the application over the mod_wsgi. with content:
    ...
    ```
     $ sudo touch catalog.wsgi && nano catalog.wsgi
    ```
    ...
    from project.py import app as application
    ```
...
#### 14. Edit the default Virtual File with following content:
  ...
  ```
  <VirtualHost *:80>
    ServerName 35.159.30.174
    ServerAdmin mohamed.edrees0@gmail.com
